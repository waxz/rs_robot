...
pub fn setup_shaders_render<M>(mut app: &mut App, systems: impl IntoSystemConfigs<M>)
{
    app.add_plugins(CustomMaterialPlugin)
        ...
        .add_systems(Startup, systems.after(create_render_resource))
        .add_systems(
            Update,
            create_shaders_buffer.run_if(run_if_shader_need_initialise),
        );
}
pub fn create_render_resource(mut commands: Commands)
{
...
pub(crate) struct CustomMaterialPlugin;
